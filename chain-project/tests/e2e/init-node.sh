...
# Set client config
bananad config keyring-backend "$KEYRING"
bananad config chain-id "$CHAINID"

# if $KEY exists it should be deleted
bananad keys add "$KEY" --keyring-backend $KEYRING --algo "$KEYALGO"

# Set moniker and chain-id for Evmos (Moniker can be anything, chain-id must be an integer)
bananad init "$MONIKER" --chain-id "$CHAINID"
...
node_address=$(bananad keys list | grep  "address: " | cut -c12-)
...
bananad add-genesis-account $KEY 100000000000000000000000000aevmos --keyring-backend $KEYRING
...
bananad gentx $KEY 1000000000000000000000aevmos --keyring-backend $KEYRING --chain-id "$CHAINID"
## In case you want to create multiple validators at genesis
## 1. Back to `bananad keys add` step, init more keys
## 2. Back to `bananad add-genesis-account` step, add balance for those
## 3. Clone this ~/.bananad home directory into some others, let's say `~/.clonedEvmosd`
## 4. Run `gentx` in each of those folders
## 5. Copy the `gentx-*` folders under `~/.clonedEvmosd/config/gentx/` folders into the original `~/.bananad/config/gentx`

# Collect genesis tx
bananad collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
bananad validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
bananad start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aevmos --json-rpc.api eth,txpool,personal,net,debug,web3
